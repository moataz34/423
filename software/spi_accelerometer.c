...
void SPI_Init(u32 base_addr)
{
    ...
    SPI_WriteReg(base_addr, SPI_ACCELEROMETER_CONTROL_REG_OFFSET, control_val);
...
void SPI_WaitTxReady(u32 base_addr)
{
    ...
    do {
        status = SPI_ReadReg(base_addr, SPI_ACCELEROMETER_STATUS_REG_OFFSET);
        timeout--;
        if (timeout == 0) {
...
void SPI_WriteByte(u32 base_addr, u8 reg_addr, u8 data)
{
    ...
    SPI_WriteReg(base_addr, SPI_ACCELEROMETER_TX_COUNT_REG_OFFSET, 2);
    ...
    SPI_WriteReg(base_addr, SPI_ACCELEROMETER_TX_DATA_REG_OFFSET, cmd_byte);

    // Wait for TX ready
    SPI_WaitTxReady(base_addr);

    // Send data byte
    SPI_WriteReg(base_addr, SPI_ACCELEROMETER_TX_DATA_REG_OFFSET, data);
...
u8 SPI_ReadByte(u32 base_addr, u8 reg_addr)
{
    ...
    SPI_WriteReg(base_addr, SPI_ACCELEROMETER_TX_DATA_REG_OFFSET, 0x00);
    ...
    // Read received data
    rx_data = SPI_ReadReg(base_addr, SPI_ACCELEROMETER_RX_DATA_REG_OFFSET) & 0xFF;
...
void SPI_ReadMultiBytes(u32 base_addr, u8 reg_addr, u8 *data, u8 count)
{
    ...
    SPI_WriteReg(base_addr, SPI_ACCELEROMETER_TX_COUNT_REG_OFFSET, count + 1);
    ...
    for (i = 0; i < count; i++) {
        ...
        // Read received data
        data[i] = SPI_ReadReg(base_addr, SPI_ACCELEROMETER_RX_DATA_REG_OFFSET) & 0xFF;
    }
