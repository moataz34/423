...
│   ├── spi_master.v
│   ├── spi_accel_v1_0_S00_AXI.v
│   └── spi_accel_v1_0.v
├── constraints/                      ← FPGA pin constraints
│   └── spi_accel_constraints.xdc
...
1. **Create Custom IP Block:**
   ...
   - `hdl/spi_master.v` → Add as source file in IP
   - `hdl/spi_accel_v1_0_S00_AXI.v` → Replace default AXI file
   - `hdl/spi_accel_v1_0.v` → Replace top-level module

2. **Add Constraints:**
   ```
   Right-click in Sources → Add Sources → Add or create constraints
   ```
   - Add `constraints/spi_accel_constraints.xdc`
...
2. **Add Files to `src/` folder:**
   ```
   YourProject/src/
   ├── spi_accel.h          ← Copy from software/
   ├── spi_accel.c          ← Copy from software/
   └── main.c               ← Copy from software/
   ```

3. **Update Base Address:**
   - Open `xparameters.h` (auto-generated)
   - Find `XPAR_SPI_ACCEL_0_S00_AXI_BASEADDR`
...
3. Add source files (spi_accel.h, spi_accel.c, main.c)
4. Build project
...
1. ✅ `spi_accel_v1_0.v` (top-level module)
2. ✅ `spi_accel_v1_0_S00_AXI.v` (AXI interface with register comments)
3. ✅ `spi_accel.h` (driver header)
4. ✅ `spi_accel.c` (driver implementation)
...
- `spi_master.v` (core SPI module)
- `spi_accel_constraints.xdc` (pin constraints)
