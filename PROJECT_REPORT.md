...
### 1. SPI Master Core Module (`spi_master.v`)
...
### 2. AXI-Lite Slave Interface (`spi_accel_v1_0_S00_AXI.v`)
...
### 3. Top-Level Wrapper (`spi_accel_v1_0.v`)
...
### 4. Constraints File (`spi_accel_constraints.xdc`)
...
### 2. Driver Implementation (`spi_accel.c`)
...
/home/user/423/
├── hdl/
│   ├── spi_master.v                    # SPI Master core module
│   ├── spi_accel_v1_0_S00_AXI.v       # AXI-Lite slave interface
│   └── spi_accel_v1_0.v               # Top-level wrapper
├── constraints/
│   └── spi_accel_constraints.xdc      # Pin assignments
├── software/
│   ├── spi_accel.h                    # Driver header
│   ├── spi_accel.c                    # Driver implementation
...
1. **Create Custom IP:**
   ...
   - Add the Verilog files to the IP:
     - `spi_master.v` → hdl directory
     - `spi_accel_v1_0_S00_AXI.v` → hdl directory
     - `spi_accel_v1_0.v` → top-level module
...
3. **Add Constraints:**
   - Add `spi_accel_constraints.xdc` to constraints folder
...
1. **Application Project:**
   ...
   - Add `spi_accel.h` to src folder
   - Add `spi_accel.c` to src folder
...
7. **Add Constraints**
   - Import `spi_accel_constraints.xdc`
...
```c
u8 id = ADXL345_ReadDeviceID(SPI_ACCEL_BASEADDR);
